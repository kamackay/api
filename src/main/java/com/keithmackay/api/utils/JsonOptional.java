...
public final class JsonOptional<T extends JsonElement> {
  /**
   * Common instance for {@code empty()}.
   */
  private static final JsonOptional<?> EMPTY = new JsonOptional<>();
   ...
   *
   * @implNote Generally only one empty instance, {@link JsonOptional#EMPTY},
   * should exist per VM.
   */
   ...
   * @throws NullPointerException if value is {@code null}
   */
  public static <T extends JsonElement> JsonOptional<T> of(T value) {
    return new JsonOptional<T>(value);
  }
  ...
  public static <T extends JsonElement> JsonOptional<T> ofNullable(T value) {
    return value == null || value.isJsonNull() ? empty() : of(value);
  }
   ...
   * URI if one exists.
   */
  public <U extends JsonElement> JsonOptional<U> map(Function<? super T, ? extends U> mapper) {
  ...
  public <U> Optional<U> mapOut(Function<? super T, ? extends U> mapper) {
    Objects.requireNonNull(mapper);
    if (!isPresent()) {
   ...
   */
  public <U extends JsonElement> JsonOptional<U> flatMap(Function<? super T, ? extends JsonOptional<? extends U>> mapper) {
    Objects.requireNonNull(mapper);
    if (!isPresent()) {
      return empty();
    } else {
  ...
  public T orElseGet(Supplier<? extends T> supplier) {
    return this.isPresent() ? value : supplier.get();
  }
