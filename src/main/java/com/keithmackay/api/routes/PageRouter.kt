...
internal constructor(
  ...
  private val ipDebouncer = Debouncer<String>(Duration.ofMinutes(1))
      ...
      when {
        result.matchedCount == 0L || body.getBoolean("ignoreRevisit", false) -> {
          ...
          if (!ip.matches(Regex("^10\\."))) {
            emailSender.send(
              title.orElseGet { model.getTitle() },
